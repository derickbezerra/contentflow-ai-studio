...
/* ── FilterPill ─────────────────────────────────────────── */
function FilterPill({
  label, selected, delay,
}: { label: string; selected: boolean; delay: number }) {
  const frame = useCurrentFrame();
...
/* ── FilterRow ──────────────────────────────────────────── */
function FilterRow({
  label, options, selectedIndex, rowDelay,
...
const SLIDES = [
  {
    badge: "1 / 5",
    title: "Colesterol alto em jovens",
    body: "Mulheres jovens também têm risco. Um exame de sangue simples pode revelar algo que ninguém suspeitava.",
    cta: "Peça o exame na próxima consulta →",
  },
  {
    badge: "2 / 5",
    title: "5 hábitos que disparam o LDL",
    body: "Sedentarismo, ultraprocessados e estresse crônico antes dos 25 anos já formam placas arteriais silenciosas.",
    cta: "Identifique qual é o seu →",
  },
  {
    badge: "3 / 5",
    title: "O exame veio alterado. E agora?",
    body: "LDL acima de 130 mg/dL em jovens pede avaliação cardiológica. Não ignore o número — age antes dos sintomas.",
    cta: "Agende sua consulta →",
  },
];

function SlideCard({
  slide, delay, gi, highlight,
}: { slide: typeof SLIDES[0]; delay: number; gi: number; highlight: boolean }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
...
function ComplianceCard({ delay }: { delay: number }) {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const enter = smooth(frame, fps, delay);
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 8.5, color: "rgba(255,255,255,0.30)", fontFamily: "system-ui,sans-serif" }}>
            Resolução CFM 1974/2011 · Código de Ética Médica
          </p>
        </div>
        <div style={{
...
/* ── Main composition — 820 frames ≈ 27s ─────────────────
   Timeline:
     0–55:   badge + headline
    55–408:  form card (filtros → tema → gerar)
      65:    Especialidade row
     102:    Especialidade médica row (Cardiologia)
     140:    Tipo de paciente row
     178:    Gênero row
     214:    Faixa etária row (18–25)
     250:    Tema input + typewriter (260)
     370:    Gerar glows
     385:    Click effect
    395–425: AI dots
    420–625: 3 slide cards (6.8s dwell — card[0] highlighted)
    620–760: compliance card + checklist (4.7s)
    755–820: council badges + hold
──────────────────────────────────────────────────────────*/
    ...
    <AbsoluteFill style={{
      ...
      {showForm && (
        <div style={{
          ...
          <FilterRow
            label="Especialidade"
            options={["Medicina", "Nutrição", "Odontologia", "Psicologia"]}
            selectedIndex={0}
            rowDelay={65}
          />
          <FilterRow
            label="Especialidade médica"
            options={["Clínica Geral", "Cardiologia", "Dermatologia"]}
            selectedIndex={1}
            rowDelay={102}
          />
          <FilterRow
            label="Tipo de paciente"
            options={["Estético", "Com dor", "Preventivo", "Crônico", "Premium"]}
            selectedIndex={2}
            rowDelay={140}
          />
          <FilterRow
          ...
          {frame >= 250 && (
            <div style={{
              ...
              <div style={{
                ...
                {frame >= 260
                  ? <Typewriter text="Colesterol alto em jovens" startFrame={260} fps={fps} />
      ...
      {showAI && (
        <div style={{
          ...
          Gerando conteúdo para Medicina · CFM…
        </div>
      )}
